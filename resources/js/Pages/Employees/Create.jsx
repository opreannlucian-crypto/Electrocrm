...
export default function Create() {
  const { data, setData, post, processing, errors } = useForm({
    name: '', cnp: '', phone: '', email: '', position: '', department: '', hire_date: '',
    salary: '', employment_type: 'full_time', is_primary_job: true, monthly_norm_hours: '166.67',
    dependent_count: '0', tax_facility_code: '', schedule: '', status: 'Activ', active: true, notes: '',
  });
  const submit = (event) => { event.preventDefault(); post(route('employees.store')); };
  ...
  return <AuthenticatedLayout><Head title="Angajat nou" />
    <div className="mx-auto max-w-5xl px-4 py-8 sm:px-6 lg:px-8">
      <div className="mb-6"><p className="text-xs font-bold uppercase tracking-wider text-blue-700">Resurse umane</p><h1 className="text-3xl font-bold text-slate-900">Angajat nou</h1><p className="mt-1 text-sm text-slate-500">Datele salariale de mai jos pot fi preluate în fluturașul lunar.</p></div>
      <form onSubmit={submit} className="space-y-6">
        ...
        <section className="rounded-2xl border border-blue-100 bg-blue-50/40 p-6 shadow-sm"><h2 className="text-lg font-bold text-slate-900">Configurare salarială</h2><p className="mt-1 text-sm text-slate-600">Salariul este tratat ca brut lunar. Ratele și facilitățile se validează în fluturaș de către persoana responsabilă.</p><div className="mt-5 grid gap-4 md:grid-cols-2">{field('salary', 'Salariu brut lunar (RON)', 'number')}{field('monthly_norm_hours', 'Normă lunară de ore', 'number', 'Valoare inițială: 166,67 ore; ajustabilă pentru perioadă.')}{field('dependent_count', 'Persoane în întreținere', 'number')}{field('tax_facility_code', 'Cod facilitate fiscală', 'text', 'Ex.: cod intern confirmat de contabilitate.')}
        ...
        <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm"><h2 className="text-lg font-bold text-slate-900">Statut și observații</h2><div className="mt-5 grid gap-4 md:grid-cols-2"><div><label className="text-sm font-semibold text-slate-700">Status</label><select value={data.status} onChange={(event) => setData('status', event.target.value)} className="mt-1 w-full rounded-lg border-slate-300 text-sm shadow-sm"><option value="Activ">Activ</option><option value="Concediu">Concediu</option><option value="Plecat">Plecat</option></select></div><label className="mt-7 flex items-center gap-3 text-sm font-semibold text-slate-700"><input type="checkbox" checked={data.active} onChange={(event) => setData('active', event.target.checked)} className="rounded border-slate-300 text-blue-600 focus:ring-blue-500" />Angajat activ</label></div><div className="mt-4"><label className="text-sm font-semibold text-slate-700">Observații</label><textarea value={data.notes} onChange={(event) => setData('notes', event.target.value)} className="mt-1 w-full rounded-lg border-slate-300 text-sm shadow-sm" rows="3" /></div></section>
        <button disabled={processing} className="rounded-xl bg-blue-600 px-5 py-3 text-sm font-bold text-white shadow-sm hover:bg-blue-700 disabled:opacity-60">Salvează angajatul</button>
      </form>
    </div>
  </AuthenticatedLayout>;
}
