    ...
    return <AuthenticatedLayout header={<div><h2 className="text-xl font-semibold leading-tight text-gray-800">Revizii</h2><p className="mt-1 text-sm text-gray-500">Termene pentru reviziile de efracție și incendiu.</p></div>}>
        ...
        <div className="py-8"><div className="mx-auto max-w-7xl space-y-6 px-4 sm:px-6 lg:px-8">
            ...
            <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                <div className="mb-4"><h1 className="text-lg font-bold text-slate-900">Filtrare revizii</h1><p className="mt-1 text-sm text-slate-500">Găsește rapid reviziile după client, termen sau stare.</p></div>
                <form onSubmit={applyFilters} className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4">
                    ...
                    <label className="block text-sm font-semibold text-slate-700">De la<input type="date" value={filterForm.data.date_from} onChange={(event) => filterForm.setData("date_from", event.target.value)} className="mt-1 w-full rounded-xl border-slate-300 focus:border-blue-500 focus:ring-blue-500" /></label>
            ...
            <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                <div className="mb-4 flex items-start justify-between gap-3"><div><h1 className="text-lg font-bold text-slate-900">{editing ? "Editează revizia" : "Adaugă revizie"}</h1><p className="mt-1 text-sm text-slate-500">Termenul următor se calculează automat.</p></div>{editing && <button type="button" onClick={clearForm} className="shrink-0 rounded-xl border border-slate-300 px-3 py-2 text-xs font-bold text-slate-700 hover:bg-slate-50">Renunță</button>}</div>
                <form onSubmit={submit} className="space-y-4">
                    <label className="block text-sm font-semibold text-slate-700">Client<ClientAutocomplete clients={clients} clientId={data.client_id} value={data.client_name} onChange={(value) => setData({ ...data, client_id: value.client_id, client_name: value.client_name })} error={errors.client_id || errors.client_name} /></label>
                    <div className="grid grid-cols-2 gap-3"><label className="block text-sm font-semibold text-slate-700">Tip<select value={data.type} onChange={(event) => setData("type", event.target.value)} className="mt-1 w-full rounded-xl border-slate-300 focus:border-blue-500 focus:ring-blue-500"><option value="efractie">Efracție</option><option value="incendiu">Incendiu</option></select>{errors.type && <p className="mt-1 text-sm text-rose-600">{errors.type}</p>}</label><label className="block text-sm font-semibold text-slate-700">Periodicitate<select value={data.period} onChange={(event) => setData("period", event.target.value)} className="mt-1 w-full rounded-xl border-slate-300 focus:border-blue-500 focus:ring-blue-500"><option value="trimestriala">Trimestrială</option><option value="semestriala">Semestrială</option><option value="anuala">Anuală</option><option value="la_cerere">La cerere</option></select>{errors.period && <p className="mt-1 text-sm text-rose-600">{errors.period}</p>}</label></div>
                    ...
                    <button disabled={processing} className="w-full rounded-xl bg-blue-600 px-5 py-2.5 text-sm font-bold text-white shadow-sm hover:bg-blue-700 disabled:opacity-50">{processing ? "Se salvează…" : editing ? "Salvează modificările" : "+ Adaugă revizie"}</button>
                </form>
            </section>
            </div>
