...
export default function Create({
    ...
    const initialClient = clients.find((client) => String(client.id) === String(selectedClientId));
    ...
    function handleClientNameChange(value) {
        const matchingClient = clients.find(
            (client) => client.name.trim().toLocaleLowerCase("ro-RO") === value.trim().toLocaleLowerCase("ro-RO")
        );

        setData({
            ...data,
            client_name: value,
            client_id: matchingClient ? String(matchingClient.id) : "",
        });
    }

    const filteredClients = useMemo(() => {
        const query = data.client_name.trim().toLocaleLowerCase("ro-RO");
        ...
        return clients
            .filter((client) => client.name.toLocaleLowerCase("ro-RO").includes(query))
            .slice(0, 8);
                    ...
                    <div className="overflow-hidden rounded-2xl bg-white shadow-xl">
                        ...
                        <div className="border-b border-slate-200 px-6 py-7 sm:px-8">
                            ...
                            <p className="mt-2 text-slate-500">
                                Completează datele lucrării și programarea
                                intervenției.
                            </p>
                        </div>
                            ...
                            <div className="px-6 py-7 sm:px-8">
                                ...
                                <div className="mb-6">
                                            ...
                                            <div className="absolute z-30 mt-1 max-h-60 w-full overflow-y-auto rounded-xl border border-slate-200 bg-white p-1 shadow-xl">
                                                {filteredClients.map((client) => (
                                                    <button
                                                        ...
                                                        className="block w-full rounded-lg px-3 py-3 text-left text-sm font-medium text-slate-700 transition hover:bg-blue-50 hover:text-blue-800"
                                                    >
                                                        {client.name}
                                                        {client.phone && <span className="ml-2 text-xs font-normal text-slate-400">{client.phone}</span>}
                                                    </button>
                                                ))}
                                            </div>
                                        )}
                                    ...
                                    <p className="mt-2 text-xs text-slate-500">
                                        Alege un client din sugestii sau scrie unul nou. Clientul nou se salvează automat în nomenclator la emiterea lucrării.
                                    </p>

                                    {(errors.client_id || errors.client_name) && (
                                        <p className="mt-1 text-sm text-red-600">
                                            {errors.client_name || errors.client_id}
                                        </p>
                                    )}
                                </div>
                                ...
                                <div className={`mb-6 overflow-hidden rounded-2xl border border-emerald-200 bg-emerald-50/50 ${isTechnician ? "hidden" : ""}`}>
